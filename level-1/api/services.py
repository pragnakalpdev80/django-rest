...
#             response = requests.get(f'{self.base_url}/{endpoint}/{housename}', timeout=10)
#             # response.raise_for_status()
#             # print(response.json())
#             return response.json()
...
class ExternalAPIService:
    ...
    def get_characters(self):
        return self.make_request('api/characters')

    def get_hogwarts_staff(self):
        return self.make_request('api/characters/staff')

    def get_characters_by_house(self, house):
        return self.make_request(f'api/characters/house/{house}')
